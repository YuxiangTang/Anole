...
def augment_ill(img, illumination, aug_color):
        ill_aug = (np.random.random(3) - 0.5) * aug_color + 1
...
def augment_img(ldr, angle, scale_range, input_size):
    angle = (random.random() - 0.5) * angle
    scale = math.exp(random.random() * math.log(scale_range[1] / scale_range[0])) * scale_range[0]
